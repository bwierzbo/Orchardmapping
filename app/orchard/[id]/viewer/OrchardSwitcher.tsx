  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      {open && (
        <ul
          role="listbox"
          className="absolute top-full right-0 mt-2 min-w-[240px] max-w-[80vw] bg-white rounded-xl shadow-xl border border-gray-200 py-1 z-30"
        >
          {orchards.map((o) => (
            <li key={o.id}>
              <button
                ...
                className={`w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 ${
                  o.id === currentId ? 'font-semibold text-green-800' : 'text-gray-800'
                }`}
              >
                <span className="block">{o.name}</span>
                <span className="block text-xs text-gray-500">{o.location}</span>
              </button>
            </li>
          ))}
        </ul>
